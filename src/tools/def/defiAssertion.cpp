...
void defiAssertion::clear() {
  ...
  for (i = 0; i < this->numItems_; i++) {
    if (this->itemTypes_[i] == 'p') {
      ...
      defiError("ERROR (DEFPARSE-6005): Internal error in Assertion #1");
    }
...
void defiAssertion::setWiredlogic(const char* name, double dist) {
  ...
  if (this->isDelay_)
    defiError("ERROR (DEFPARS-6006): both WIREDLOGIC and DELAY statements are defined in constraint/assertion.\nThis is illegal");
  this->isWiredlogic_ = 1;
...
void defiAssertion::setSum() {
  if (this->isDiff_)
     defiError("ERROR (DEPPARS-6007): both SUM and DIFF statements are defined in constraint/assertion.\nThis is illegal");
  this->isSum_ = 1;
}

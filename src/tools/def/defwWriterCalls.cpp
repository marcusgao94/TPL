...
int defwWrite(FILE* f, const char* fName, defiUserData uData) {
  ...
  if (defwHasInit == 0 && defwHasInitCbk == 0) {
    fprintf(stderr, "ERROR DEFWRIT-9010): defwWrite is called before defwInitCbk.\nThis is illegal.");
    return -1;
  }

  if (defwHasInit) {
    fprintf(stderr, "ERROR DEFWRIT-9011): defwInit was used to initialize the writer.\n");
    fprintf(stderr, 
...
void defwPrintUnusedCallbacks(FILE* f) {
  ...
  if (defwRegisterUnused == 0) {
    fprintf(f,
    "ERROR DEFWRIT-9012): defwSetRegisterUnusedCallbacks was not called to setup this data.\n");
    return;
  }

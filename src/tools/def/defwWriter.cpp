...
#define DEFW_COMPONENT_START  32
#define DEFW_COMPONENT        33
#define DEFW_COMPONENT_END    34
#define DEFW_PIN_START        35
#define DEFW_PIN              36
#define DEFW_PIN_PORT         37
#define DEFW_PIN_END          38
#define DEFW_PINPROP_START    39
#define DEFW_PINPROP          40
#define DEFW_PINPROP_END      41
#define DEFW_BLOCKAGE_START   42
#define DEFW_BLOCKAGE_LAYER   43
#define DEFW_BLOCKAGE_PLACE   44
#define DEFW_BLOCKAGE_RECT    45
#define DEFW_BLOCKAGE_POLYGON 46
#define DEFW_BLOCKAGE_END     47
#define DEFW_SNET_START       48
#define DEFW_SNET             49
#define DEFW_SNET_OPTIONS     50
#define DEFW_SNET_ENDNET      51
#define DEFW_SNET_END         52
#define DEFW_PATH_START       53
#define DEFW_PATH             54
#define DEFW_SHIELD           55
#define DEFW_NET_START        56
#define DEFW_NET              57
#define DEFW_NET_OPTIONS      58
#define DEFW_NET_ENDNET       59
#define DEFW_NET_END          60
#define DEFW_SUBNET           61
#define DEFW_NOSHIELD         62
#define DEFW_IOTIMING_START   63
#define DEFW_IOTIMING         64
#define DEFW_IOTIMING_END     65
#define DEFW_SCANCHAIN_START  66
#define DEFW_SCANCHAIN        67
#define DEFW_SCAN_FLOATING    68
#define DEFW_SCAN_ORDERED     69
#define DEFW_SCANCHAIN_END    70
#define DEFW_FPC_START        71
#define DEFW_FPC              72
#define DEFW_FPC_OPER         73
#define DEFW_FPC_OPER_SUM     74
#define DEFW_FPC_END          75
#define DEFW_GROUP_START      76
#define DEFW_GROUP            77
#define DEFW_GROUP_END        78 
#define DEFW_SLOT_START       79
#define DEFW_SLOT_LAYER       80
#define DEFW_SLOT_RECT        81
#define DEFW_SLOT_END         82
#define DEFW_FILL_START       83
#define DEFW_FILL_LAYER       84
#define DEFW_FILL_VIA         85
#define DEFW_FILL_OPC         86
#define DEFW_FILL_RECT        87
#define DEFW_FILL_END         88
#define DEFW_NDR_START        89
#define DEFW_NDR              90
#define DEFW_NDR_END          91
#define DEFW_STYLES_START     92
#define DEFW_STYLES           93
#define DEFW_STYLES_END       94
#define DEFW_BEGINEXT_START   95
#define DEFW_BEGINEXT         96
#define DEFW_BEGINEXT_END     97 
...
int defwDieAreaList(int num_points, int* xl, int* yl) {
  ...
  if (num_points < 4) return DEFW_BAD_DATA;
...
int defwStartDefaultCap(int count) {
  ...
  if ((defwState >= DEFW_DEFAULTCAP_START) &&
      (defwState <= DEFW_DEFAULTCAP_END)) return DEFW_BAD_ORDER;
...
int defwEndDefaultCap() {
  ...
  if (defwState != DEFW_DEFAULTCAP_START &&
      defwState != DEFW_DEFAULTCAP) return DEFW_BAD_ORDER;
  if (defwCounter > 0) return DEFW_BAD_DATA;
  else if (defwCounter < 0) return DEFW_TOO_MANY_STMS;
...
int defwViaPattern(const char* pattern) {
  ...
  if (defwViaHasVal) return DEFW_ALREADY_DEFINED;  // either PatternName or
                                                   // ViaRule has defined
...
int defwViaPolygon(const char* layerName,
  ...
  for (i = 0; i < num_polys; i++) {
     if ((i == 0) || ((i % 5) != 0))
        fprintf(defwFile, "( %.11g %.11g ) ", *xl++, *yl++);
     else {
        fprintf(defwFile, "\n                ( %.11g %.11g ) ", *xl++, *yl++);
        defwLines++;
     }
...
int defwViaViarule(const char* viaRuleName, double xCutSize, double yCutSize,
...
int defwViaViaruleOffset(int xBotOffset, int yBotOffset,
...
int defwComponentHalo(int left, int bottom, int right, int top) {
  ...
  if (defVersionNum < 5.6)
      return DEFW_WRONG_VERSION;
  if (defwState != DEFW_COMPONENT) return DEFW_BAD_ORDER;
...
// 5.7
int defwComponentRouteHalo(int haloDist, const char* minLayer,
                           const char* maxLayer) {
  defwFunc = DEFW_COMPONENT;   // Current function of writer
  if (! defwFile) return DEFW_UNINITIALIZED;
  if (defVersionNum < 5.7)
...
int defwPinLayer(const char* layerName, int spacing, int designRuleWidth,
                 int xl, int yl, int xh, int yh) {
  if (! defwFile) return DEFW_UNINITIALIZED;
  ...
  if (defVersionNum < 5.6)
      return DEFW_WRONG_VERSION;
  if (spacing && designRuleWidth) return DEFW_BAD_DATA;  // only one, spacing
...
int defwPinPolygon(const char* layerName, int spacing, int designRuleWidth,
                   int num_polys, double* xl, double* yl) {
  int i;
...
// 5.7
int defwPinVia(const char* viaName, int xl, int yl) {
...
// 5.7
int defwPinPortLayer(const char* layerName, int spacing, int designRuleWidth,
                 int xl, int yl, int xh, int yh) {
  if (! defwFile) return DEFW_UNINITIALIZED;
...
// 5.7
int defwPinPortPolygon(const char* layerName, int spacing, int designRuleWidth,
                   int num_polys, double* xl, double* yl) {
  int i;
...
// 5.7
int defwPinPortVia(const char* viaName, int xl, int yl) {
...
// 5.7
int defwPinPortLocation(const char* status, int statusX, int statusY,
                        const char* orient) {
  int i;
...
int defwSpecialNetShieldNetName(const char* name) {
  defwFunc = DEFW_PATH;   // Current function of writer
  if (! defwFile) return DEFW_UNINITIALIZED;
  if (defwState != DEFW_PATH_START) return DEFW_BAD_ORDER;
...
int defwSpecialNetPathPointWithWireExt(int numPts, const char** pointx,
                     const char** pointy, const char** value) {
  int i;
  defwFunc = DEFW_PATH;   // Current function of writer
  if (! defwFile) return DEFW_UNINITIALIZED;
  if (defwState != DEFW_PATH) return DEFW_BAD_ORDER;
...
int defwSpecialNetRect(const char* layerName,
                       int xl, int yl, int xh, int yh) {
  defwFunc = DEFW_SNET_OPTIONS;   // Current function of writer
  if (! defwSpecialNetOptions() &&
       (defwState != DEFW_PATH))  // not inside a path
...
int defwStartIOTimings(int count) {
  ...
  if ((defwState >= DEFW_IOTIMING_START) &&
      (defwState >= DEFW_IOTIMING_END)) return DEFW_BAD_ORDER;
  if (defVersionNum >= 5.4) return DEFW_OBSOLETE;
...
int defwStartScanchains(int count) {
  ...
  if ((defwState >= DEFW_SCANCHAIN_START) &&
      (defwState <= DEFW_SCANCHAIN_END)) return DEFW_BAD_ORDER;
...
int defwBlockagePolygon(int num_polys, double* xl, double* yl) {
  int i;
...
int defwNonDefaultRuleLayer(const char* layerName, int width,
                            int diagWidth, int spacing, int wireExt) {
  defwFunc = DEFW_NDR;
  if (! defwFile) return DEFW_UNINITIALIZED;
  // May be user already define layer
  if (defwState != DEFW_NDR)
...
int defwStyles(int styleNums, int num_points, double* xp, double* yp) {
  ...
  for (i = 0; i < num_points; i++) {
     if ((i == 0) || ((i % 5) != 0))
        fprintf(defwFile, "( %.11g %.11g ) ", *xp++, *yp++);
     else {
        fprintf(defwFile, "\n       ( %.11g %.11g ) ", *xp++, *yp++);
        defwLines++;
     }

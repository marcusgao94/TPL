...
int lefwLayerCutSpacing(double spacing) {
  ...
  if (lefwState != LEFW_LAYER_START &&
     lefwState != LEFW_LAYER) return LEFW_BAD_ORDER;
  if (lefwIsLayerCutSpacing) return LEFW_BAD_ORDER;
...
int lefwLayerCutSpacingCenterToCenter() {
  if (!lefwIsLayerCutSpacing) return LEFW_BAD_ORDER; // has to be after 
...
// 5.7
int lefwLayerCutSpacingLayer(const char* name2, int stack) {   /* optional */
  if (!lefwIsLayerCutSpacing) return LEFW_BAD_ORDER; // has to be after 
                                  // lefwLayerCutSpacing is called
  fprintf(lefwFile, "\n     LAYER %s", name2);
  if (stack) 
...
// 5.7
int lefwLayerCutSpacingAdjacent(int viaCuts,    /* either 2, 3 or 4, optional */
                                double distance,
                                int except) { /* optional */
  if (!lefwIsLayerCutSpacing) return LEFW_BAD_ORDER; // has to be after 
                                  // lefwLayerCutSpacing is called
  if (viaCuts < 2 && viaCuts > 4) return LEFW_BAD_DATA;
...
// 5.7
int lefwLayerRoutingMinimumcutWithin(double numCuts, double minWidth,
                                     double cutDistance) {
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
      lefwState != LEFW_LAYERROUTING) return LEFW_BAD_ORDER;
...
int lefwLayerRoutingSpacingEndOfLine(double eolWidth, double eolWithin) {
  ...
  if (lefwState != LEFW_LAYERROUTING_START &&
      lefwState != LEFW_LAYERROUTING) return LEFW_BAD_ORDER;
  if (!lefwIsRouting) return LEFW_BAD_DATA;  // only routing calls this func
  if (lefwHasLayerRoutingEol) return LEFW_BAD_DATA;// already defined in Spacing
  if (prtSemiColon) {
     // the previous statement hasn't written the ; yet
     fprintf(lefwFile, ";\n");
     prtSemiColon = 0;
...
// 5.7
int lefwLayerRoutingSpacingEOLParallel(double parSpace, double parWithin,
                                       int twoEdges) {
  if (!lefwHasLayerRoutingEol) return LEFW_BAD_DATA;// EndOfLine in not defined
  if (twoEdges)
...
// 5.7
int lefwLayerRoutingSpacingEndOfNotchWidth(double eonWidth, double minNSpacing,
                                           double minNLength) {
  if (!lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_START &&
      lefwState != LEFW_LAYERROUTING) return LEFW_BAD_ORDER;
...
int lefwLayerRoutingSpacingtableTwoWidthsWidth(double width, double runLength,
                                      int numSpacing, double* spacing) {
  int i;
  if (! lefwFile) return LEFW_UNINITIALIZED;
  if (lefwState != LEFW_LAYERROUTING_SP_START &&
...
// 5.7
int lefwLayerAntennaAreaDiffReducePwl(int numPwls, double* diffAreas,
  ...
  if (lefwState != LEFW_LAYERROUTING_START && lefwState != LEFW_LAYERROUTING
      ...
      return LEFW_BAD_ORDER;
  if (!lefwIsRouting && !lefwIsCut) return LEFW_BAD_DATA;
  if (numPwls < 2) return LEFW_BAD_DATA;

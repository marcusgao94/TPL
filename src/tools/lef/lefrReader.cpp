...
void lefrEnableParserMsgs (int nMsg, int* msgs) {
  ...
  for (i = 0; i < nMsg; i++) {     /* loop through the given list */
     for (j = 0; j < nDMsgs; j++) {
        if (disableMsgs[j] == msgs[i]) {
           disableMsgs[j] = -1;    /* temp assign a -1 on that slot */ 
           break;
        }
     }
  } 
